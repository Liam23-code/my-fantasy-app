"""Safe, offline CFB injury-data loading -- our own file plus user uploads, never a live fetch.

Mirrors modules.injury_parser's loading functions (``load_injury_data_from_file``,
``load_injury_data_from_user_upload``) exactly, substituting college
team-name normalization for NBA's fixed 30-team alias table. The
status/impact math (``normalize_status``, ``severity_impact_score``, and
friends) has no sport in it at all -- imported directly from
...
DEFAULT_INJURIES_PATH = DATA_ROOT / "cfb_injuries.json"
...
def walk_injury_records(node: Any, team: str, output: list[dict[str, Any]], *, source: str = "offline") -> None:
    """Recursively parse an already-obtained, arbitrarily-nested CFB injury payload."""
...
def load_injury_data_from_file(path: str | Path | None = None) -> list[dict[str, Any]]:
    """Load a real CFB injury report from a local JSON file (default: ``data/cfb_injuries.json``).
...
def load_injury_data_from_user_upload(source: Any, *, file_format: str | None = None) -> list[dict[str, Any]]:
    """Parse a user-uploaded CFB injury file: JSON, CSV, or delimited text."""
    name = ""
