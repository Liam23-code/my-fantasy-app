"""MLB game odds (moneyline): our own default file plus user uploads -- never a sportsbook fetch.

Mirrors modules.cbb_odds_loader's shape and alias-based normalization,
substituting MLB's fixed 30-team alias table (modules.mlb_common). MLB
has no spread market (run line is out of scope this cycle) -- only
moneyline and total are modeled, the same fields modules.mlb_moneyline_model
consumes. ``data/mlb_game_odds.json`` ships empty by design: this app
never fetches game odds from any sportsbook, and no live MLB schedule
ingestion was added this cycle (see mlb_pipeline.md).
"""
...
from modules.mlb_common import normalize_team_name

DATA_ROOT = Path(__file__).resolve().parents[1] / "data"
DEFAULT_GAME_ODDS_PATH = DATA_ROOT / "mlb_game_odds.json"
...
def load_default_game_odds(path: str | Path | None = None) -> dict[str, Any]:
    """Load our default MLB game-odds file (default: ``data/mlb_game_odds.json``).
...
def load_uploaded_game_odds(source: Any, *, file_format: str | None = None) -> dict[str, Any]:
    """Parse a user-uploaded MLB game-odds file: JSON, CSV, or delimited text."""
...
def unified_game_odds(
    *, default_path: str | Path | None = None, uploaded: Any = None, uploaded_format: str | None = None
    ...
    """Our default MLB game odds, optionally merged with an uploaded override."""
