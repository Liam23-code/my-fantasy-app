"""CFB game odds (moneyline/spread/total): our own default file plus user uploads.

Mirrors modules.nba_odds_loader's shape and alias-based normalization
exactly, substituting college team-name normalization (no fixed alias
table -- see modules.college_sports_common) for NBA's 30-team one.
"""
...
DEFAULT_GAME_ODDS_PATH = DATA_ROOT / "cfb_game_odds.json"
...
def _normalize_game_row(row: dict[str, Any], *, source: str) -> dict[str, Any] | None:
    home = normalize_college_team_name(str(_first(row, _HOME_TEAM_ALIASES) or ""))
    away = normalize_college_team_name(str(_first(row, _AWAY_TEAM_ALIASES) or ""))
...
def load_default_game_odds(path: str | Path | None = None) -> dict[str, Any]:
    """Load our default CFB game-odds file (default: ``data/cfb_game_odds.json``).
...
def load_uploaded_game_odds(source: Any, *, file_format: str | None = None) -> dict[str, Any]:
    """Parse a user-uploaded CFB game-odds file: JSON, CSV, or delimited text."""
...
def unified_game_odds(
    *, default_path: str | Path | None = None, uploaded: Any = None, uploaded_format: str | None = None
    ...
    """Our default CFB game odds, optionally merged with an uploaded override."""
