"""CFB moneyline/spread/total model: win probability and fair prices from real team scoring.

Mirrors modules/nba_moneyline_model.py exactly: reuses
``betting.team_model.project_game`` (a pure "your real scoring rate vs.
opponent's real rate allowed" blend -- no sport-specific constant baked
in) and every function in ``betting.odds_math`` directly. The only
sport-specific piece is the margin/total standard deviation, from
:mod:`modules.cfb_team_model`'s real, computed CFB volatility rather than
the NFL's published constant or NBA's own computed one -- college
football's real variance is a different number from both.
"""
...
from modules.cfb_team_model import real_margin_and_total_volatility
...
def win_probability_from_spread(spread: float, *, stdev: float) -> float:
    """P(the favored-by-``spread`` team wins), from the normal approximation to CFB margins."""
...
def _apply_team_status_penalty(
    ...
    """Shade each team's expected points down by a caller-supplied points penalty
    (unavailable key players -- see
    :func:`fantasy.multi_sport_status.team_status_penalty`), then recompute the
    spread and total. ``None`` / ``{}`` -> the projection is returned unchanged,
    so this is a pure opt-in seam -- mirrors ``betting.moneyline_model`` for NFL
    exactly."""
...
def fair_moneyline(
    ...
    *,
    ...
    """Fair spread, total, and moneyline price for one matchup, from real CFB team scoring.

    ``team_status_penalty`` optionally maps an upper-cased team code to a points
    penalty for unavailable key players; it shades that team's expected points
    before the win-probability math (which is otherwise untouched). Omit it for
    the historical behaviour exactly.
    """
    projection = project_game(home_team, away_team, averages=averages)
    projection = _apply_team_status_penalty(
        projection, home_team.strip().upper(), away_team.strip().upper(), team_status_penalty
    )
...
def evaluate_game(
    ...
    *,
    ...
    ``team_status_penalty`` is passed straight through to :func:`fair_moneyline`
    -- an optional ``{TEAM: points}`` map (from
    :func:`fantasy.multi_sport_status.team_status_penalty`) for unavailable key
    players. Omit it for the exact historical behaviour.
    """
    volatility = volatility or real_margin_and_total_volatility(0)
    home_team, away_team = game_odds["home_team"], game_odds["away_team"]
    fair = fair_moneyline(
        home_team,
        away_team,
        averages=averages,
        margin_stdev=volatility["margin_stdev"],
...
def evaluate_games(
    ...
    *,
    ...
    """Evaluate real CFB matchups, optionally compared against loaded CFB game odds.
    ...
    for game in games:
        ...
        rows.append(
            evaluate_game(
                game_odds, averages=averages, volatility=volatility, team_status_penalty=team_status_penalty
            )
        )
