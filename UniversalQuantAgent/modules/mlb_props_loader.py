"""MLB player prop lines: our own default file plus user uploads -- never a sportsbook fetch.

Mirrors modules.cbb_props_loader's default-file-plus-upload convention
exactly, substituting MLB's fixed 30-team alias table and its own 7-category
vocabulary (modules.mlb_common) for CBB's college normalizer and
basketball categories. ``data/mlb_props.json`` ships empty by design (see
its own ``"note"`` field and mlb_pipeline.md): no live, keyless MLB stats
API was integrated this cycle (unlike CFB/CBB's ESPN/CFBD exception --
see offline_data_contract.md), so real per-game rates come only from a
file you upload here.
"""
...
from modules.mlb_common import normalize_category, normalize_player_name, normalize_team_name
from modules.sportsbook_parser import now_iso

DATA_ROOT = Path(__file__).resolve().parents[1] / "data"
DEFAULT_PROPS_PATH = DATA_ROOT / "mlb_props.json"
_DEFAULT_FILE_CACHE_SECONDS = 300

_PLAYER_ALIASES = ("player_name", "player", "name", "athlete")
_TEAM_ALIASES = ("team", "mlb_team")
...
def load_props_from_file(path: str | Path | None = None) -> list[dict[str, Any]]:
    """Load our own default MLB prop lines (default: ``data/mlb_props.json``).
...
def load_props_from_user_upload(source: Any, *, file_format: str | None = None) -> list[dict[str, Any]]:
    """Parse a user-uploaded MLB prop-lines file: JSON, CSV, or delimited text."""
    name = ""
...
def unified_props(
    *, default_path: str | Path | None = None, uploaded: Any = None, uploaded_format: str | None = None
    ...
    """Our default MLB prop lines, optionally merged with an uploaded override."""
