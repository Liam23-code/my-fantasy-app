"""CFB player prop lines: our own default file plus user uploads -- never a sportsbook fetch.

Mirrors modules.nba_props_loader's default-file-plus-upload convention
exactly (see that module and offline_data_contract.md). The one real
difference: college team names have no small, fixed alias table the way
NBA's 30 teams do (see modules.college_sports_common), and the stat
categories are football's, not basketball's.
"""
...
from modules.sportsbook_parser import now_iso

DATA_ROOT = Path(__file__).resolve().parents[1] / "data"
DEFAULT_PROPS_PATH = DATA_ROOT / "cfb_props.json"
_DEFAULT_FILE_CACHE_SECONDS = 300

#: category -> aliases a real or user-supplied row might spell it as.
_CATEGORY_ALIASES_BY_CANONICAL: dict[str, frozenset[str]] = {
    "passing_yards": frozenset({"passing_yards", "pass_yards", "passyards", "passingyards"}),
    "passing_tds": frozenset({"passing_tds", "pass_tds", "passingtds"}),
    "rushing_yards": frozenset({"rushing_yards", "rush_yards", "rushingyards"}),
    "rushing_tds": frozenset({"rushing_tds", "rush_tds", "rushingtds"}),
    "receiving_yards": frozenset({"receiving_yards", "rec_yards", "receivingyards"}),
    "receiving_tds": frozenset({"receiving_tds", "rec_tds", "receivingtds"}),
    "receptions": frozenset({"receptions", "rec", "catches"}),
}
_CATEGORY_LOOKUP: dict[str, str] = {
    alias: canonical for canonical, aliases in _CATEGORY_ALIASES_BY_CANONICAL.items() for alias in aliases
}

_PLAYER_ALIASES = ("player_name", "player", "name", "athlete")
_TEAM_ALIASES = ("team", "cfb_team")
...
def normalize_category(value: str) -> str | None:
    key = "".join(ch for ch in str(value).strip().lower() if ch.isalnum() or ch == "_").replace(" ", "_")
...
def load_props_from_file(path: str | Path | None = None) -> list[dict[str, Any]]:
    """Load our own default CFB prop lines (default: ``data/cfb_props.json``).
...
def load_props_from_user_upload(source: Any, *, file_format: str | None = None) -> list[dict[str, Any]]:
    """Parse a user-uploaded CFB prop-lines file: JSON, CSV, or delimited text."""
    name = ""
...
def unified_props(
    *, default_path: str | Path | None = None, uploaded: Any = None, uploaded_format: str | None = None
    ...
    """Our default CFB prop lines, optionally merged with an uploaded override."""
