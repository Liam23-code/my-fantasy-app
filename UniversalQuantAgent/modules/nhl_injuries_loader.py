"""Safe, offline NHL injury-data loading -- our own file plus user uploads, never a live fetch.

Mirrors modules.mlb_injuries_loader exactly, substituting NHL's fixed
team alias table (modules.nhl_common). The status/impact math has no
sport in it at all -- imported directly from :mod:`modules.injury_parser`
rather than duplicated. ``data/nhl_injuries.json`` ships empty by design.
"""
...
from modules.nhl_common import normalize_player_name, normalize_team_name

DATA_ROOT = Path(__file__).resolve().parents[1] / "data"
DEFAULT_INJURIES_PATH = DATA_ROOT / "nhl_injuries.json"
...
def walk_injury_records(node: Any, team: str, output: list[dict[str, Any]], *, source: str = "offline") -> None:
    """Recursively parse an already-obtained, arbitrarily-nested NHL injury payload."""
...
def load_injury_data_from_file(path: str | Path | None = None) -> list[dict[str, Any]]:
    """Load a real NHL injury report from a local JSON file (default: ``data/nhl_injuries.json``).
...
def load_injury_data_from_user_upload(source: Any, *, file_format: str | None = None) -> list[dict[str, Any]]:
    """Parse a user-uploaded NHL injury file: JSON, CSV, or delimited text."""
    name = ""
