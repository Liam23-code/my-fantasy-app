"""NHL player prop lines: our own default file plus user uploads -- never a sportsbook fetch.

Mirrors modules.mlb_props_loader's default-file-plus-upload convention
exactly, substituting NHL's fixed 32-team alias table and 4-category
vocabulary (modules.nhl_common). ``data/nhl_props.json`` ships empty by
design: no live or keyless NHL stats source was integrated this cycle
(see nhl_pipeline.md).
"""
...
from modules.nhl_common import normalize_category, normalize_player_name, normalize_team_name
from modules.sportsbook_parser import now_iso

DATA_ROOT = Path(__file__).resolve().parents[1] / "data"
DEFAULT_PROPS_PATH = DATA_ROOT / "nhl_props.json"
_DEFAULT_FILE_CACHE_SECONDS = 300

_PLAYER_ALIASES = ("player_name", "player", "name", "athlete")
_TEAM_ALIASES = ("team", "nhl_team")
...
def load_props_from_file(path: str | Path | None = None) -> list[dict[str, Any]]:
    """Load our own default NHL prop lines (default: ``data/nhl_props.json``).
...
def load_props_from_user_upload(source: Any, *, file_format: str | None = None) -> list[dict[str, Any]]:
    """Parse a user-uploaded NHL prop-lines file: JSON, CSV, or delimited text."""
    name = ""
...
def unified_props(
    *, default_path: str | Path | None = None, uploaded: Any = None, uploaded_format: str | None = None
    ...
    """Our default NHL prop lines, optionally merged with an uploaded override."""
