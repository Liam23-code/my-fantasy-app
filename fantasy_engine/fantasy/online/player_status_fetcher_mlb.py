"""player_status_fetcher_mlb: refresh the ``mlb`` slice of ``multi_sport_status.json``.

The MLB sibling of :mod:`fantasy.online.player_status_fetcher` (NFL). Same
...
clicks **Refresh Player Status** on the MLB betting page.

Source: ESPN's public, keyless MLB injuries feed
(``https://site.api.espn.com/apis/site/v2/sports/baseball/mlb/injuries``). A
starting pitcher or an everyday bat on the injured list is the case this is here
to catch; the feed is naturally quiet between seasons, and a quiet or failed
pull simply leaves the overlay as it was.
"""
...
def normalize_feed(payload: Any, *, now: str | None = None) -> dict[str, dict[str, Any]]:
    """ESPN MLB injuries JSON -> the unified status map. ``{}`` on junk input."""
...
def refresh_player_status(
    ...
    *,
    ...
    """Fetch the MLB feed and rewrite only the ``mlb`` slice of ``path``.
