"""player_status_fetcher_nhl: refresh the ``nhl`` slice of ``multi_sport_status.json``.

The NHL sibling of :mod:`fantasy.online.player_status_fetcher` (NFL). Same
...
clicks **Refresh Player Status** on the NHL betting page.

Source: ESPN's public, keyless NHL injuries feed
(``https://site.api.espn.com/apis/site/v2/sports/hockey/nhl/injuries``). A goalie
ruled out or a top-line skater on IR is the case this catches; a quiet or failed
...
def normalize_feed(payload: Any, *, now: str | None = None) -> dict[str, dict[str, Any]]:
    """ESPN NHL injuries JSON -> the unified status map. ``{}`` on junk input."""
...
def refresh_player_status(
    ...
    *,
    ...
    """Fetch the NHL feed and rewrite only the ``nhl`` slice of ``path``.
