...
def _generate_qb(rng: np.random.Generator, index: int, team: str) -> dict:
    ...
    return {
        "player_id": f"synthetic:qb:{index}",
        "name": f"Synthetic QB {index}",
        "position": "QB",
        "team": team,
        "opponent": rng.choice(NFL_TEAMS),
        "season": 2026,
        "passing_yards": passing_yards,
        "passing_tds": passing_tds,
        "interceptions": _clip_normal(rng, 0.5, 0.3),
        "rushing_yards": rushing_yards,
        "rushing_tds": _clip_normal(rng, 0.15, 0.15),
        "receptions": 0.0,
        "receiving_yards": 0.0,
        "receiving_tds": 0.0,
        "fumbles_lost": _clip_normal(rng, 0.1, 0.08),
        "median": median,
        "floor": round(median * 0.55, 2),
        "ceiling": round(median * 1.65, 2),
...
def _generate_rb(rng: np.random.Generator, index: int, team: str) -> dict:
    ...
    return {
        "player_id": f"synthetic:rb:{index}",
        "name": f"Synthetic RB {index}",
        "position": "RB",
        "team": team,
        "opponent": rng.choice(NFL_TEAMS),
        "season": 2026,
        ...
        "interceptions": 0.0,
        "rushing_yards": rushing_yards,
        "rushing_tds": _clip_normal(rng, 0.35, 0.25),
        "receptions": receptions,
        "receiving_yards": receiving_yards,
        "receiving_tds": _clip_normal(rng, 0.08, 0.1),
        "fumbles_lost": _clip_normal(rng, 0.05, 0.05),
        "median": median,
        "floor": round(median * 0.5, 2),
        "ceiling": round(median * 1.7, 2),
...
def _generate_receiver(rng: np.random.Generator, index: int, team: str, position: str) -> dict:
    ...
    return {
        "player_id": f"synthetic:{position.lower()}:{index}",
        "name": f"Synthetic {position} {index}",
        "position": position,
        "team": team,
        "opponent": rng.choice(NFL_TEAMS),
        "season": 2026,
        ...
        "receptions": receptions,
        "receiving_yards": receiving_yards,
        "receiving_tds": _clip_normal(rng, 0.35 * scale, 0.2),
        "fumbles_lost": _clip_normal(rng, 0.03, 0.04),
        "median": median,
        "floor": round(median * 0.45, 2),
        "ceiling": round(median * 1.8, 2),
        "drivers": ["target_share", "air_yards"],
    }
...
def main() -> None:
    ...
    roster = [
        ...
        for pid, p, slot in zip(roster_player_ids, players[:9], slots, strict=True)
    ]
